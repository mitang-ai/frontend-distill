...
`playwright`는 자동 브라우저 접근에 사용되고, `npx playwright install chromium`은 실제 브라우저 실행 파일을 설치하는 단계입니다.
...
1. agent 또는 도구가 대상 사이트 URL을 직접 엽니다.
2. 자동 추출 명령을 실행해 스크린샷, raw extraction, bundle, tokens를 생성합니다.
3. 생성된 결과를 `frontend-distill` skill에 전달합니다.
...
- 페이지를 연다
- 지연 로딩 콘텐츠가 나타나도록 스크롤한다
- extractor를 주입한다
- 스크린샷을 저장한다
- `raw-extraction.json`을 출력한다
- `extraction-bundle.json`을 출력한다
- `design-tokens.json`을 출력한다
- `layout-tokens.json`을 출력한다
...
DevTools에 [`tools/browser/extract_design_tokens.js`](./tools/browser/extract_design_tokens.js)를 붙여 넣는 방식은 이제 보조용 fallback입니다.
